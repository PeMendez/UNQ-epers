...
class PatogenoServiceTest {
    ...
    fun recuperarPatogenoTest(){
        ...
        var patogenoRecuperado = patogenoService.crearPatogeno(virus)
        Assert.assertEquals(virus.id, patogenoRecuperado.id)
        Assert.assertEquals(virus.tipo, patogenoRecuperado.tipo)
        Assert.assertEquals(virus.cantidadDeEspecies, patogenoRecuperado.cantidadDeEspecies)
        Assert.assertEquals(virus.capacidadDeContagio, patogenoRecuperado.capacidadDeContagio)
        Assert.assertEquals(virus.capacidadDeBiomecanizacion, patogenoRecuperado.capacidadDeBiomecanizacion)
    }


    //este test deberia estar funcionando, falta retocar la implemetacion de agregar especie
    @Test
    fun seLeAgregaUnaEspecieAUnPatogenoTest(){
        var bacteria = Patogeno("Bacteria")
        patogenoService.crearPatogeno(bacteria)
        var especieGenerada = patogenoService.agregarEspecie(bacteria.id!!,"EspecieViolenta", 2)
        var listaEspecies = patogenoService.especiesDePatogeno(bacteria.id!!)
        Assert.assertTrue(listaEspecies.contains(especieGenerada))
    }
